  ...
  d3.selectAll(".data")
    .filter(function (d) {
      ...
      if ("properties" in item) {
        ...
        else return item.properties.name == d.title;
      } else if ("properties" in d) {
        return item.title == d.properties.name;
      } else {
...
function handleMouseOut(event, item) {
  ...
  const colorScale = d3
    .scaleLog()
    .domain([
      d3.min(currentData, (d) => d.pages),
      d3.max(currentData, (d) => d.pages),
    ])
    .range([0, 1]);
  ...
  currentData.forEach((element) => {
    d3.selectAll(".country.data")
      .filter(function (d) {
        return d.properties.name == element.pages;
      })
